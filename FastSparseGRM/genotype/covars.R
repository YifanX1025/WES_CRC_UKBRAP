...
data <- read.csv("age_sex_PC1_10.csv")

# Check the structure of the data
str(data)
head(data)
colnames(data)[4:13] <- paste0("PC", 1:10)
...
covars <- covar %>%
  ...
  select(eid, age, sex, PC1, PC2, PC3, PC4, PC5, PC6, PC7, PC8, PC9, PC10) %>%
  ...
  select(eid, age_c, age2_c, sex, age_c_sex, age2_c_sex,
